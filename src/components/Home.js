...
const Home = () => {
  ...
  const checkInput = (event) => {
    ...
    if (event.data === " ") {
      if (
        latestWord.length < letterElementLength ||
        latestWord !== words[currentWordIndex].textContent
      ) {
      ...
      if (
        latestWord === "" &&
        (words[currentWordIndex - 1].classList.contains("underline-text") ||
          isMobile)
      ) {
        // or inputBox.value.trim() !== ''
        if (isHighlightingEnabled) {
          highlightWord(true);
        }
        setLatestWord(typedWords[currentWordIndex - 1]);
        words[currentWordIndex - 1].classList.remove("underline-text");
        setCurrentWordIndex((prevCurrentWordIndex) =>
          Math.max(prevCurrentWordIndex - 1, 0)
        );
        return;
      }
      ...
      if (
        ...
        if (
          letterElement[letterElementLength - 1].textContent.match(
            punctuationPattern
          ) &&
          typedWords[currentWordIndex - 1].match(punctuationPattern)
        ) {
          const lastWord = inputBoxRef.current.value.trim().split(" ").pop();
          setTotalTyped(
            (prevTotalTyped) =>
              prevTotalTyped - latestWord.length + lastWord.length
          );
        ...
        if (latestWordLength >= letterElementLength) {
          setLastLetterRect(
            letterRects[currentWordIndex - 1][letterElementLength - 1]
          );
        } else {
          setLastLetterRect(
            letterRects[currentWordIndex - 1][latestWordLength - 1]
          );
        }
        return;
      }
  ...
  const updateWord = (latestWord, i, backspaceFlag = false) => {
    ...
    if (latestWord === "" && backspaceFlag) {
      ...
      if (
        lastLetterRect !==
        letterRects[currentWordIndex][Math.max(latestWord.length - 1, 0)]
      ) {
        setLastLetterRect(
          latestWord.length > letterElementLength - 1
            ? letterRects[currentWordIndex][letterElementLength - 1]
            : letterRects[currentWordIndex][Math.max(latestWord.length - 1, 0)]
        );
      } else {
        setLastLetterRect(
  ...
  const endTest = (endTime) => {
    ...
    const newAccuracy = calculateLocalAccuracy(
      accuracy,
      newTotalRacesTaken,
      profileData.totalAvgAccuracy
    );
    const newWpm = calculateLocalWpm(
      netWPM,
      newTotalRacesTaken,
      profileData.totalAvgWpm
    );
    try {
      updateDoc(userRef, {
  ...
  const changeColors = () => {
    const newColor = document.body.classList.contains("dark")
      ? "#18191A"
      : "#E4E9F7";
  ...
  const repeatTest = () => {
    ...
    changeColors();
    setCursorStyle({
      left: `${letterRects[0][0].left}px`,
      top: `${letterRects[0][0].top}px`,
    });
    for (let i = 0; i < words.length; i++) {
      words[i].classList.remove("underline-text");
      const letterElements = words[i].querySelectorAll(".letter");
  ...
  const calculateAccuracy = (totalTyped, totalErrors) => {
    const accuracy = Math.round(
      ((totalTyped - totalErrors) / totalTyped) * 100
    );
  ...
  const loadImages = () => {
    levels.forEach((image) => {
      ...
      try {
        ...
        console.error(
          `Failed to load image for level ${image.title}: ${error}`
        );
      }
    });
  };
  ...
  const updateCursorPosition = () => {
    ...
    const letterRect =
      newLetterRects[currentWordIndex][Math.max(latestWord.length - 1, 0)];
    setCursorStyle({
  ...
  useEffect(() => {
    if (quoteRef.current) {
      ...
      if (letterRects[0]) {
        setCursorStyle({
          top: letterRects[0][0].top,
          left: letterRects[0][0].left,
        });
      }
    }
  }, [quoteDivs]);
  ...
  useEffect(() => {
    ...
    let accuracy = 0;
    if (
      latestWord[latestWord.length - 1] !==
      letterElement[Math.min(latestWord.length - 1, letterElement.length - 1)]
        .textContent
    ) {
  ...
  useEffect(() => {
    ...
    for (let i = currentWordIndex; i <= lastWordIndex; i++) {
      isHighlightingEnabled
        ? words[i].classList.add("inactive-word")
  ...
  useEffect(() => {
    ...
    if (currentWordIndex === lastWordIndex) {
      if (latestWord.length >= letterElements[currentWordIndex].length) {
        ...
        latestWord.trim();
        if (
          latestWord.length < currentWord.length ||
            ...
            .then((doc) => {
              if (doc.exists()) {
                ...
                setProfileData({
                  ...
                  profilePhotoUrl:
                    data[process.env.REACT_APP_PROFILE_PHOTO_URL_KEY],
                  totalRacesTaken:
                    data[process.env.REACT_APP_TOTAL_RACES_TAKEN_KEY],
                  totalAvgAccuracy:
                    data[process.env.REACT_APP_TOTAL_AVG_ACCURACY_KEY],
  ...
  return (
    <div className={`App`}>
      <div className="container">
        <Header
          toBeFocusedRef={inputBoxRef}
          smoothCursorBlockRef={smoothCursorBlockRef}
        />
        <ToastContainer
        ...
        {isCursorHidden ? (
          ...
          <span className={`cursor`} ref={cursorRef} style={cursorStyle}></span>
        )}
        ...
        <div>
          ...
          <button className="button" id="repeatButton" onClick={repeatTest}>
            Repeat
          </button>
          <button className="button" id="customButton" onClick={openModal}>
            Custom Text
          </button>
          {clearButton ? (
            <button
        ...
        <div className="stats">
          <div className="stat">
            <div ref={wpmDisplayRef} id="wpmDisplay">
              Current WPM: {wpm}
            </div>
          </div>
          <div className="stat">
          ...
          <div className="stat">
            <div ref={errorsDisplayRef} id="errorsDisplay">
              Errors: {totalErrors}
            </div>
          </div>
        </div>
        ...
        <div className="stats">
          <div className="stat">
            <div
              ...
              ref={smoothCursorBlockRef}
              onClick={handleSmoothCursorChange}
            >
              Smooth Caret:{" "}
              <span
                className={isSmoothCursorOn ? "correct" : "incorrect"}
                onClick={handleSmoothCursorChange}
              >
          ...
          <div className="stat">
            <div
              ...
              ref={highlightedWordsElementRef}
              onClick={handleHighlightingChange}
            >
              Highlighting:{" "}
              <span
                className={isHighlightingEnabled ? "correct" : "incorrect"}
                onClick={handleHighlightingChange}
              >
        ...
        <div className="stats">
          <div className="stat">
            <div id="categoryDisplay" ref={categoryDisplayRef}>
              {category}
            </div>
            {isCapsLockOn ? (
