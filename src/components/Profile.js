...
function Profile(props) {
  ...
  useEffect(() => {
    if (profileData) {
      ...
      setTotalRacesTaken(profileData.totalRacesTaken);
      setTotalAvgAccuracy(profileData.totalAvgAccuracy);
      setTotalAverageWpm(profileData.totalAvgWpm);
    }
  }, [profileData]);
  ...
  useEffect(() => {
    ...
    if (
      profilePhotoUrl !== "" &&
      profilePhotoUrl !== process.env.REACT_APP_DEFAULT_PROFILE_PHOTO_URL
    ) {
      const userRef = doc(
        db,
      ...
      updateDoc(userRef, {
        [process.env.REACT_APP_PROFILE_PHOTO_URL_KEY]: profilePhotoUrl,
      }).catch((error) => {
        console.error(
          "Error updating profile photo URL in the database:",
          error
        );
      });
      setIsLoading(false);
